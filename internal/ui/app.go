...
func (a *App) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	// All remaining message types are owned by Phase 4 reducers
	// (see internal/ui/reducers.go for the dispatch chain). Anything
	// not handled by the reducers falls through to the no-op tail
	// below. tea.WindowSizeMsg, tea.KeyMsg, tea.MouseWheelMsg, and
	// tea.MouseClickMsg are the only message types still handled in
	// this switch -- they are mouse/keyboard routers that
	// orchestrate too many sub-models to absorb into a single
	// reducer (MouseClick + MouseWheel will move to reducer_mouse.go
	// in Phase 4m).
	}
